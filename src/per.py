...
class PrioritizedReplayBuffer:
    def __init__(self, batch_size: int, buffer_size: Union[int, float], alpha: float, seed: int,
                 device: torch.device):
        ...
        self.states = None
        self.actions = None
        self.dones = np.zeros((self.buffer_size,), dtype=np.long)
    ...
    def sample(self):
        ...
        dones = torch.tensor(dones, device=self.device, dtype=torch.uint8).unsqueeze(1)
        
        return states, actions, rewards, next_states, dones, nodes, samples_id
    
    def calc_is_weight(self, nodes_value, beta: float):
        """
